...
/**
 ...
 @param cubeId cubeid
 */
- (void)onConferenceAddStream:(CubeConference *)conference cubeId:(NSString *)cubeId;
...
/**
 ...
 @param cubeId cubeid
 */
- (void)onConferenceRemoveStream:(CubeConference *)conference cubeId:(NSString *)cubeId;

#pragma mark - 会议更新通知
/**
 引擎内部自行维护了当前用户所在会议集合(包含会议对象生命周期内所有变更,创建、加入、推出、销毁、变更等等)
 @note 回调数据为 conferenceId、groupId集合(方便查询)
 @dicuss 使用conferenceWithGroupId、conferenceWithConferenceId查询
 @param updates conferenceId、groupId集合
 */
- (void)onConferenceManageUpdate:(NSArray *)updates;
...
/**
 ...
 @param failure 失败回调
 */
-(void)queryConferenceWithConferenceType:(int )conferenceTypes
...
-(void)queryConferenceWithCubeId:(NSString *)cubeId
                  ConferenceType:(int )conferenceTypes
                      completion:(void(^)(NSArray *conferences))completion
                         failure:(void(^)(CubeError *error))failure;
...
#pragma mark - query update conference api

/**
 根据回调接口中更新的查询条件进行查询
 @note 详见onConferenceManageUpdate中的描述
 @param groupId 根据群组id查询会议
 @return 查询会议结果,若不存在会议,即表示该会议已结束并已销毁
 */
-(nullable CubeConference *)conferenceWithGroupId:(nonnull NSString *)groupId;

/**
 同上

 @param conferenceId 根据会议id查询会议
 @return 查询会议结果
 */
-(nullable CubeConference *)conferenceWithConferenceId:(nonnull NSString *)conferenceId;
