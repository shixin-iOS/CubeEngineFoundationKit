...
#pragma mark - 白板更新通知
/**
 引擎内部自行维护了当前用户所在会议集合(包含白板对象生命周期内所有变更,创建、加入、推出、销毁、变更等等)
 @note 回调数据为 whiteboardId、groupId集合(方便查询)
 @dicuss 使用whiteBoardWithGroupId、whiteBoardWithWhiteBoardId查询
 @param updates whiteboardId、groupId集合
 */
- (void)onWhiteBoardManageUpdate:(NSArray *)updates;
...
/**
 ...
 @param cubeId 用户cubeId
 @param success 成功回调
 @param failure 失败回调
 */
- (void)syncWhiteBoardWithCubeId:(NSString *)cubeId
                         success:(void(^)(NSDictionary *data))success
                         failure:(void(^)(CubeError *error))failure;
...
/**
 ...
 @param failure 失败回调
 */
- (void)syncWhiteBoardWithGroupIds:(NSArray *)groupIds
                           success:(void(^)(NSDictionary *data))success
                           failure:(void(^)(CubeError *error))failure;

//////////// 多白板策略暂不考虑 /////////

#pragma mark - 白板更新查询,详见onWhiteBoardManageUpdate回调

-(nullable CubeWhiteBoard *)whiteBoardWithGroupId:(nonnull NSString *)groupId;

-(nullable CubeWhiteBoard *)whiteBoardWithWhiteBoardId:(nonnull NSString *)whiteBoardId;
